#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>

#include "aes.h"

const u8 s_box[256] = {
    
};
 
const u8 mixColMat[BLOCK_SIDE][BLOCK_SIDE] = {
    { 0x02, 0x03, 0x01, 0x01 },
    { 0x01, 0x02, 0x03, 0x01 },
    { 0x01, 0x01, 0x02, 0x03 },
    { 0x03, 0x01, 0x01, 0x02 }
};

u8 galoisMul(u8 g1, u8 g2) {
    u8 poly = 0;

    for (int i = 0; i < 8; i++) {
        if (g2 & 0x01) poly ^= g1;
        bool msb = g1 & 0x80; // g1 >= 128 = 0100 0000
        g1 << 1;
        if (msb) g1 ^= AES_IRREDUCIBLE; // g1 -= 00011011 == mod (x^8+x^4+x^3+x+1)
        g2 << 1;
    }

    return poly;
}

void AddRoundKey(u8 state[BLOCK_SIDE][BLOCK_SIDE], u8 roundKey[BLOCK_SIDE][BLOCK_SIDE]) {
    for (int r = 0; r < BLOCK_SIDE; r++) {
        for (int c = 0; c < BLOCK_SIDE; c++)
            state[r][c] ^= roundKey[r][c];
    }
}
void SubByte(u8 state[BLOCK_SIDE][BLOCK_SIDE]) {
    for (int r = 0; r < BLOCK_SIDE; r++) {
        for (int c = 0; c < BLOCK_SIDE; c++)
            state[r][c] = s_box[state[r][c]];
    }
}
void ShiftRows(u8 state[BLOCK_SIDE][BLOCK_SIDE]) {
    for (int r = 0; r < BLOCK_SIDE; r++)
        leftRotate(state[r], r, BLOCK_SIDE);
}
void MixColumns(u8 state[BLOCK_SIDE][BLOCK_SIDE]) {
    u8 res[BLOCK_SIDE][BLOCK_SIDE];
    
    for (int r = 0; r < BLOCK_SIDE; r++) {
        for (int c = 0; c < BLOCK_SIDE; c++) {
            res[r][c] = 0x00;

            for (int i = 0; i < BLOCK_SIDE; i++)
                res[r][c] ^= galoisMul(mixColMat[r][i], state[i][c]);
        }
    }

    memcpy(state, res, BLOCK_SIDE * BLOCK_SIDE * sizeof(u8));
}

void KeySchedule(u8* ukey, int keyLen, u8 roundKeys[][BLOCK_SIDE][BLOCK_SIDE]) {
    KeySchedule128(ukey, roundKeys);
}
void KeySchedule128(u8* ukey, u8 roundKeys[11][BLOCK_SIDE][BLOCK_SIDE]) {
    int i = 0;
    for (int c = 0; c < BLOCK_SIDE; c++) {
        for (int r = 0; r < BLOCK_SIDE; r++)
            roundKeys[0][r][c] = ukey[i++];
    }

    u8 rCon = 0x01;
    /*
        Round 1: x^0 = 1
        Round 2: x^1 = x
        Round 3: x^2 =
        Round 9: x^8 := x^8 mod p(x)
    */
    for (i = 1; i < 10; i++) {
        u8 g[4] = {
            s_box[roundKeys[i-1][1][3]] ^ rCon,
            s_box[roundKeys[i-1][2][3]],
            s_box[roundKeys[i-1][3][3]],
            s_box[roundKeys[i-1][0][3]]
        };

        for (int r = 0; r < BLOCK_SIDE; r++)
            roundKeys[i][r][0] = roundKeys[i-1][r][0] ^ g[r];

        for (int c = 1; c < BLOCK_SIDE; c++) {
            for (int r = 0; r < BLOCK_SIDE; r++)
                roundKeys[i][r][c] = roundKeys[i-1][r][c] ^ roundKeys[i][r][c-1];
        }
    }

    rCon = galoisMul(rCon, 0x02);
}

void aes_encrypt_block(u8* input, int n,
                        u8 roundKeys[][BLOCK_SIDE][BLOCK_SIDE], int Nr,
                        u8 output[BLOCK_LEN]) {
    u8 state[BLOCK_SIDE][BLOCK_SIDE];
    int i = 0;
    for (int c = 0; c < BLOCK_SIDE; c++) {
        for (int r = 0; r< BLOCK_SIDE; r++)
            state[r][c] = (i < n) ? input[i++] : 0;
    }

    // ROUND 0
    AddRoundKey(state, roundKeys[0]);

    // MAIN ROUND
    for (i = 1; i < Nr; i++) {
        SubByte(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, roundKeys[i]);
    }

    // FINAL ROUND
    SubByte(state);
    ShiftRows(state);
    AddRoundKey(state, roundKeys[Nr]);

    i = 0;
    for (int c = 0; c < BLOCK_SIDE; c++) {
        for (int r = 0; r < BLOCK_SIDE; r++)
            output[i++] = state[r][c];
    }
}

void aes_encrypt(u8* input, int n,
                 u8* ukey, int keyLen,
                 u8** output) {
    int Nr = 10;
    switch (keyLen)
    {
    case AES_192:
        Nr = 12;
        break;
    case AES_256:
        Nr = 14;
        break;
    };

    u8 (*roundKeys)[BLOCK_SIDE][BLOCK_SIDE] =
        malloc((Nr+1) * sizeof(u8[BLOCK_SIDE][BLOCK_SIDE]));
    KeySchedule(ukey, keyLen, roundKeys);

    int noBlocks = n >> 4; // n / BLOCK_LEN
    int extra = n & 0x0F; // n % BLOCK_LEN

    int outLen = noBlocks * BLOCK_LEN;
    if (extra) {
        *output = malloc((noBlocks + 1) * BLOCK_LEN * sizeof(u8));
        outLen += BLOCK_LEN;
    } else {
        *output = malloc(noBlocks * BLOCK_LEN * sizeof(u8));
    }

    for (int i = 0; i < noBlocks; i = i << 4)
        aes_encrypt_block(input + (i << 4), BLOCK_LEN, roundKeys, Nr, *output + (i << 4));
}